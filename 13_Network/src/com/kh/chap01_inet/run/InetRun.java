...
public class InetRun {
	...
	public static void main(String[] args) {
		...
		/*
		 * 네트워크(Network) : 여러 대 컴퓨터들이 연결되어있는 통신망 같은 거
		 * 					=> 네트워크를 통해 서로 간의 데이터 교환 가능!
		 * 
		 ...
		 * 
		 * - 서버에 요청하기 위해서 그 요청하고자 하는 서버의 IP 주소(또는 도메인명), 포트번호 알아야됨!
		 */
		...
		try {
			...
			InetAddress googleHost = InetAddress.getByName("www.google.com"); // 도메인명을 
			...
			InetAddress[] naverHost = InetAddress.getAllByName("www.daum.net");
			
			for(InetAddress n : naverHost) { // n=naverHost[0] => naverHost[1]
				System.out.println("다음 서버명 : " + n.getHostName());
				System.out.println("다음 IP주소 : " + n.getHostAddress());
			}
		...
		/*
		 * * 현재 구동중인 서버가 있으면
		 * 	클라이언트는 그 서버로 요청을 보낼 수 있음! => 응답 결과 돌아옴!
		 * 
		 ...
		 * 	이때 데이터를 입출력하고자 한다면 
		 * 서버와 클라이언트간에 스트림(연결통로)이 필요함!!
		 * 
		 * * 소켓 : 프로세스 간의 통신을 담당하는 것! (스트림을 연결하기 위한 문)
		 * 
		 * * 소켓 프로그래밍 (TCP 방식/UDP 방식)
		 * - TCP 방식 : 데이터 전송속도 느림! 데이터가 정확하고 안정적으로 전달 가능함! (신뢰성이 요구되는 프로그램 : EMAIL, 잔액조회, ...)
		 * - UDP 방식 : 데이터 전송속도 빠름! 신뢰성이 없는 데이터 전달될 수 있음! (데이터를 빠른 속도로 전송하고자 하는 프로그램)
		 */
